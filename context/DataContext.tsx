...
export const DataProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    return new Promise((resolve, reject) => {
      ...
      reader.onload = (event) => {
        ...
        img.onload = () => {
          ...
          const MAX_WIDTH = 700; // Small enough for text storage
          const scaleSize = MAX_WIDTH / img.width;
          canvas.width = MAX_WIDTH;
          canvas.height = img.height * scaleSize;
          const ctx = canvas.getContext('2d');
          ctx?.drawImage(img, 0, 0, canvas.width, canvas.height);
          // High compression for reliability
  ...
  const uploadFile = async (path: string, file: File): Promise<string> => {
    ...
    // We wrap the upload in a timeout to prevent hanging if Storage is blocked by billing
    ...
    const timeoutPromise = new Promise<never>((_, reject) => 
      setTimeout(() => reject(new Error("STORAGE_TIMEOUT")), 2000)
    );

    try {
      // Race the actual upload against a 2-second timeout
      return await Promise.race([storagePromise, timeoutPromise]);
    } catch (error: any) {
      console.warn("Storage upload skipped/failed (likely billing issue). Using Base64 fallback.", error);
      // Return the base64 string directly - Firestore can store this easily
  ...
  const addLead = async (lead: StudentLead) => {
    ...
    showToast(`Student ${lead.name} has been added.`, 'success');
  };
  ...
  const addUser = async (user: User) => {
    ...
    showToast(`Staff ${user.name} added.`, 'success');
  };
  ...
  const assignLeadsToHOD = async (leadIds: string[], hodId: string) => {
    ...
    await batch.commit();
    showToast(`Assigned ${leadIds.length} leads.`, 'success');
  };
  ...
  const autoDistributeLeadsToHODs = async (leadIds: string[]) => {
    ...
    await batch.commit();
    showToast("Leads distributed to HODs.", 'success');
  };
  ...
  const autoDistributeLeadsToTeachers = async (leadIds: string[], department: Department) => {
    ...
    await batch.commit();
    showToast("Leads distributed to faculty.", 'success');
  };
  ...
  return (
    <DataContext.Provider value={{ 
      ...
      markMessagesAsSeen, addLog, exportSystemData, importSystemData, uploadFile, uploadProfileImage
    }}>
      {children}
