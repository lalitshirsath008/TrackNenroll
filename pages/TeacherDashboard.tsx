...
const TeacherDashboard: React.FC<{ currentUser: User }> = ({ currentUser }) => {
  ...
  // Leads that are either assigned to me or were delegated BY me
  ...
  // All active teachers except current user
  ...
  const handleBulkDelegate = async (targetTeacherId: string) => {
    ...
    if (window.confirm(`Transfer ${selectedHistoryIds.length} student leads to ${targetTeacher.name}?`)) {
      // Logic for bulk transfer
      ...
      addLog(currentUser.id, currentUser.name, UserAction.MANUAL_ADD, `Delegated ${selectedHistoryIds.length} leads to faculty member ${targetTeacher.name}.`);
      
      setIsDelegateModalOpen(false);
      setSelectedHistoryIds([]);
      showToast(`Successfully transferred ${selectedHistoryIds.length} leads to ${targetTeacher.name}.`, 'success');
    }
  };
        ...
        {(activeTab === 'pending' ? pendingLeads : completedLeads).map(lead => {
          ...
          return (
            <div 
              ...
              className={`bg-white rounded-[2.5rem] border-2 p-8 flex flex-col shadow-sm hover:shadow-xl transition-all group relative cursor-pointer ${isSelected ? 'border-indigo-500 bg-indigo-50/10' : 'border-slate-100'}`}
            >
              {activeTab === 'completed' && (
              ...
              <div className="mt-auto pt-4 space-y-3">
                {activeTab === 'completed' && !isSentByMe && (
                  <div className="py-3 px-6 bg-slate-50 rounded-2xl text-center text-[9px] font-black uppercase text-slate-500 border border-slate-100 mb-2">
                    Result: {lead.response || 'In Progress'}
                  </div>
                )}
          ...
          <div className="bg-white w-full max-w-md rounded-[3rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
            ...
            <div className="p-6">
              <div className="mb-6 p-5 bg-slate-50 border border-slate-100 rounded-3xl flex items-center gap-4">
                ...
                <div>
                  <p className="text-xs font-black text-slate-800 uppercase tracking-tight">Bulk Reassignment</p>
                  <p className="text-[9px] font-bold text-slate-400 tracking-widest uppercase">Moving {selectedHistoryIds.length} leads to new faculty</p>
                </div>
              </div>
            ...
            <div className="p-6 bg-slate-50 border-t border-slate-100">
              <button onClick={() => setIsDelegateModalOpen(false)} className="w-full py-3 text-[10px] font-black uppercase text-slate-400 tracking-widest">Cancel</button>
            </div>
          </div>
        </div>
      )}
            ...
            <div className="p-10 bg-[#0f172a] text-white text-center relative overflow-hidden">
              ...
              {isCallActive ? (
                ...
                <div className="px-6 py-4 bg-emerald-500/20 border border-emerald-500/30 rounded-2xl">
                  <p className="text-[9px] font-black text-emerald-400 uppercase tracking-widest">{callDuration < MIN_VALID_DURATION ? `Record ${MIN_VALID_DURATION - callDuration}s more` : 'Call Verified'}</p>
                </div>
              )}
            </div>
            ...
            <div className="p-10 space-y-6 bg-white relative">
              {isLocked && (
                <div className="absolute inset-0 bg-white/60 backdrop-blur-[2px] z-10 flex flex-col items-center justify-center text-center p-8">
                  ...
                  <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]">{isCallActive ? 'Finalizing...' : `Minimum wait: ${MIN_VALID_DURATION - callDuration}s`}</p>
                </div>
              )}
              ...
              {!showDeptPicker ? (
                <div className="space-y-4">
                  ...
                  <button onClick={() => setCallingLead(null)} className="w-full py-3 text-slate-300 text-[9px] font-black uppercase tracking-[0.2em] mt-2">Close</button>
                </div>
              ) : (
