...
const TeacherDashboard: React.FC<{ currentUser: User }> = ({ currentUser }) => {
  ...
  const [wasDialerOpened, setWasDialerOpened] = useState(false);
  ...
  const handleCategorization = (leadId: string, response: StudentResponse, selectedDept?: Department) => {
    if (callDuration < MIN_VALID_DURATION) {
      alert(`Bhai, categorization ke liye kam se kam ${MIN_VALID_DURATION} seconds wait karna hoga (Call Verification).`);
      return;
    }
  ...
  return (
    <div className="space-y-6 font-['Inter']">
      ...
      <div className="flex bg-slate-100 p-1.5 rounded-2xl w-full md:w-max">
        <button onClick={() => setActiveTab('pending')} className={`px-8 py-3.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'pending' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500'}`}>New Leads</button>
        <button onClick={() => setActiveTab('completed')} className={`px-8 py-3.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'completed' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500'}`}>History</button>
      </div>
        ...
        {(activeTab === 'pending' ? pendingLeads : completedLeads).map(lead => (
          <div key={lead.id} className="bg-white rounded-[2.5rem] border border-slate-100 p-8 flex flex-col shadow-sm hover:shadow-xl transition-all group">
            <div className="flex justify-between items-start mb-6">
              ...
              <span className={`px-4 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest ${lead.callVerified ? 'bg-emerald-50 text-emerald-600' : 'bg-amber-50 text-amber-600'}`}>{lead.callVerified ? 'Done' : 'Pending'}</span>
            </div>
            ...
            {lead.stage === LeadStage.ASSIGNED || lead.stage === LeadStage.UNASSIGNED ? (
              <button onClick={() => startCallSession(lead)} className="w-full py-5 bg-[#0f172a] text-white rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-slate-800 transition-all active:scale-[0.98] shadow-lg shadow-slate-100">Start Calling</button>
            ) : (
        ...
        <div className="fixed inset-0 z-[1000] bg-slate-900/95 backdrop-blur-xl flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-xl rounded-[3.5rem] overflow-hidden shadow-2xl flex flex-col animate-in zoom-in-95 duration-200">
            <div className="p-10 bg-[#0f172a] text-white text-center relative overflow-hidden">
              ...
              {isCallActive ? (
                <button onClick={endCallSession} className="px-12 py-5 bg-red-600 text-white rounded-2xl font-black text-[11px] uppercase tracking-widest shadow-xl shadow-red-900/40 active:scale-95 transition-all">End Call Now</button>
              ) : (
                <div className="px-6 py-4 bg-emerald-500/20 border border-emerald-500/30 rounded-2xl">
                  <p className="text-[9px] font-black text-emerald-400 uppercase tracking-widest">{callDuration < MIN_VALID_DURATION ? `Wait ${MIN_VALID_DURATION - callDuration}s to unlock` : 'Session Recorded'}</p>
                </div>
              )}
            </div>
            ...
            <div className="p-10 space-y-6 bg-white relative">
              {isLocked && (
                <div className="absolute inset-0 bg-white/60 backdrop-blur-[2px] z-10 flex flex-col items-center justify-center text-center p-8">
                  ...
                  <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]">{isCallActive ? 'Finish call first' : `Wait ${MIN_VALID_DURATION - callDuration}s to unlock`}</p>
                </div>
              )}
              ...
              {!showDeptPicker ? (
                <div className="space-y-4">
                  ...
                  <button 
                    ...
                    disabled={isLocked}
                    className="w-full py-6 bg-[#4c47f5] text-white rounded-[1.5rem] font-black text-[14px] uppercase tracking-widest shadow-xl shadow-indigo-100 transition-all hover:bg-indigo-700 active:scale-[0.98] flex items-center justify-center gap-3"
                  >
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M14.828 14.828a4 4 0 01-5.656 0M9 10h.01M15 10h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                    INTERESTED
                  </button>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <button 
                      ...
                      disabled={isLocked}
                      className="py-4 bg-slate-50 border border-slate-200 rounded-2xl text-[10px] font-black uppercase text-slate-600 hover:bg-rose-50 hover:text-rose-600 transition-all"
                    >
                      NOT INTERESTED
                    </button>
                    <button 
                      ...
                      disabled={isLocked}
                      className="py-4 bg-slate-50 border border-slate-200 rounded-2xl text-[10px] font-black uppercase text-slate-600 hover:bg-amber-50 hover:text-amber-600 transition-all"
                    >
                      CONFUSED
                    </button>
                    <button 
                      ...
                      disabled={isLocked}
                      className="py-4 bg-slate-50 border border-slate-200 rounded-2xl text-[10px] font-black uppercase text-slate-600 hover:bg-indigo-50 hover:text-indigo-600 transition-all"
                    >
                      11TH / 12TH
                    </button>
                    <button 
                      ...
                      disabled={isLocked}
                      className="py-4 bg-slate-50 border border-slate-200 rounded-2xl text-[10px] font-black uppercase text-slate-600 hover:bg-slate-200 transition-all"
                    >
                      NOT RESPONDING
                    </button>
                    <button 
                      onClick={() => handleCategorization(callingLead.id, StudentResponse.NOT_REACHABLE)} 
                      disabled={isLocked}
                      className="py-4 bg-slate-50 border border-slate-200 rounded-2xl text-[10px] font-black uppercase text-slate-600 hover:bg-slate-200 transition-all"
                    >
                      NOT REACHABLE
                  ...
                  <button onClick={() => setCallingLead(null)} className="w-full py-3 text-slate-300 text-[9px] font-black uppercase tracking-[0.2em] mt-2">Discard Session</button>
                </div>
              ) : (
                <div className="space-y-6 animate-in slide-in-from-bottom-4">
                   <div className="text-center">
                     <p className="text-[12px] font-black text-[#0f172a] uppercase tracking-tighter">Preferred Branch</p>
                     <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-1">Select branch for {callingLead.name}</p>
                   </div>
