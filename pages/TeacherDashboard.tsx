...
const TeacherDashboard: React.FC<{ currentUser: User }> = ({ currentUser }) => {
  const { leads, users, updateLead, showToast, addLog } = useData();
  ...
  // Leads tracking: Leads assigned to me OR leads I delegated to others
  ...
  // Pending: Leads currently in my queue that are not yet processed
  ...
  // History: Leads I have processed OR leads I have sent away to someone else
  ...
  const handleCategorization = async (leadId: string, response: StudentResponse, selectedDept?: Department) => {
    if (callDuration < MIN_VALID_DURATION) {
      showToast(`Minimum ${MIN_VALID_DURATION}s call required for verification.`, 'error');
      return;
    }
    ...
    addLog(currentUser.id, currentUser.name, UserAction.MANUAL_ADD, `Classified student ${callingLead?.name} as ${response}.`);
    setCallingLead(null);
  ...
  const handleBulkDelegate = async (targetTeacherId: string) => {
    ...
    if (window.confirm(`Delegate ${selectedHistoryIds.length} leads to ${targetTeacher.name}?`)) {
      try {
        // We update one by one for safety or use a batch logic if preferred. 
        // Here we ensure NO 'undefined' is passed to the updateLead function.
        for (const leadId of selectedHistoryIds) {
          await updateLead(leadId, {
            assignedToTeacher: targetTeacherId,
            stage: LeadStage.ASSIGNED,
            callVerified: false,
            // Track who sent it and where it went
        ...
        addLog(currentUser.id, currentUser.name, UserAction.MANUAL_ADD, `Bulk delegated ${selectedHistoryIds.length} leads to ${targetTeacher.name}.`);
        
        setIsDelegateModalOpen(false);
        setSelectedHistoryIds([]);
        showToast(`Successfully moved ${selectedHistoryIds.length} leads to ${targetTeacher.name}.`, 'success');
      } catch (error) {
        console.error("Delegation Error:", error);
        showToast("Delegation process failed. Please check connection.", "error");
      }
    }
  };
    ...
    <div className="space-y-6 font-['Inter'] pb-12">
      ...
      <div className="bg-[#0f172a] p-8 md:p-12 rounded-[2.5rem] text-white shadow-2xl flex flex-col md:flex-row justify-between items-center gap-8 relative overflow-hidden">
        ...
        <div className="relative z-10 text-center md:text-left">
          ...
          <p className="text-indigo-400 text-[10px] font-black uppercase tracking-[0.3em] mt-3">Welcome back, {currentUser.name}</p>
        </div>
        <div className="relative z-10 flex gap-4 w-full md:w-auto">
          <div className="flex-1 md:flex-none bg-indigo-600 px-8 py-5 rounded-3xl text-center shadow-xl border border-white/10">
          ...
          <div className="flex-1 md:flex-none bg-white/5 px-8 py-5 rounded-3xl text-center border border-white/10 backdrop-blur-md">
            ...
            <p className="text-[8px] font-black uppercase tracking-widest mt-1">Incoming</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
        <div className="flex bg-slate-100 p-1.5 rounded-2xl w-full md:w-max shadow-inner border border-slate-200">
          <button onClick={() => { setActiveTab('pending'); setSelectedHistoryIds([]); }} className={`px-8 py-3.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'pending' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500'}`}>Assigned Leads</button>
        ...
        {activeTab === 'completed' && completedLeads.length > 0 && (
          <div className="flex items-center gap-3 w-full md:w-auto">
             <button onClick={toggleSelectAllHistory} className="px-5 py-3 bg-white border border-slate-200 text-[9px] font-black uppercase rounded-xl hover:bg-slate-50 transition-all flex items-center gap-2">
                <div className={`w-4 h-4 rounded border flex items-center justify-center ${selectedHistoryIds.length === completedLeads.length ? 'bg-indigo-600 border-indigo-600' : 'border-slate-300'}`}>
                   {selectedHistoryIds.length === completedLeads.length && <svg className="w-2.5 h-2.5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="4" d="M5 13l4 4L19 7"/></svg>}
                </div>
                {selectedHistoryIds.length === completedLeads.length ? 'Deselect All' : 'Select All'}
             </button>
             {selectedHistoryIds.length > 0 && (
               <button onClick={() => setIsDelegateModalOpen(true)} className="px-8 py-3 bg-[#0f172a] text-white text-[9px] font-black uppercase rounded-xl shadow-lg animate-in slide-in-from-right-2">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            >
              ...
              <div className="flex justify-between items-start mb-6">
                ...
                <div className="flex flex-col items-end gap-1">
                  ...
                  {isSentByMe && <span className="text-[8px] font-black text-indigo-600 uppercase bg-indigo-50 px-2 py-1 rounded-lg">Delegated Out</span>}
                </div>
              </div>
              
              <h4 className="font-black text-[#0f172a] text-xl uppercase tracking-tighter truncate leading-tight">{lead.name}</h4>
              <p className="text-indigo-600 text-sm font-black tracking-widest mt-1 mb-4">{lead.phone}</p>
              
              {/* Delegation Context */}
              {(isReceivedByMe || isSentByMe) && (
                <div className="mb-4 p-3 bg-slate-50 rounded-2xl border border-slate-100 flex items-center gap-2">
                  <div className={`w-2 h-2 rounded-full ${isSentByMe ? 'bg-indigo-500 animate-pulse' : 'bg-emerald-500 animate-pulse'}`}></div>
                  <p className="text-[9px] font-black uppercase text-slate-500 tracking-tight">
                    {isSentByMe ? (
                      <span>Sent to: <span className="text-indigo-600">{lead.delegatedToName}</span></span>
                    ) : (
                      <span>From: <span className="text-emerald-600">{lead.delegatedFromName}</span></span>
                    )}
                  </p>
                </div>
              )}
              ...
              <div className="mt-auto pt-4 space-y-3">
                {activeTab === 'completed' && !isSentByMe && (
                  <div className="py-3 px-6 bg-slate-100 rounded-2xl text-center text-[9px] font-black uppercase text-slate-500 border border-slate-200 mb-2">
                    {lead.response ? `Response: ${lead.response}` : 'Awaiting Call'}
                  </div>
                )}
                ...
                <div className="flex gap-2">
                  <button 
                    ...
                    disabled={isSentByMe}
                    className={`flex-1 py-5 rounded-2xl font-black text-[10px] uppercase tracking-widest transition-all active:scale-[0.98] shadow-lg disabled:opacity-30 ${activeTab === 'pending' ? 'bg-[#0f172a] text-white hover:bg-slate-800 shadow-slate-200' : 'bg-white border-2 border-indigo-100 text-indigo-600 hover:bg-indigo-50 shadow-indigo-50'}`}
                  >
                    {activeTab === 'pending' ? 'Initiate Call' : isSentByMe ? 'Delegated' : 'Call Back'}
                  </button>
                </div>
              </div>
            </div>
        ...
        {(activeTab === 'pending' ? pendingLeads : completedLeads).length === 0 && (
          <div className="col-span-full py-24 text-center text-slate-300 text-[10px] font-black uppercase tracking-[0.3em]">
          ...
          <div className="bg-white w-full max-w-md rounded-[3rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="p-8 bg-[#0f172a] text-white flex justify-between items-center">
              <div className="text-left">
                <p className="text-[10px] font-black text-indigo-400 uppercase tracking-widest mb-1">Lead Distribution</p>
            ...
            <div className="p-6">
              <div className="mb-6 p-5 bg-slate-50 border border-slate-100 rounded-3xl flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-[9px] font-bold text-slate-400 tracking-widest uppercase">Select receiving faculty member</p>
                </div>
              </div>
              ...
              <div className="relative mb-4">
                <input 
                  ...
                  className="w-full pl-5 pr-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl text-xs font-bold outline-none focus:border-indigo-500 transition-all shadow-inner"
                />
              </div>
            ...
            <div className="p-6 bg-slate-50 border-t border-slate-100">
              <button onClick={() => setIsDelegateModalOpen(false)} className="w-full py-3 text-[10px] font-black uppercase text-slate-400 tracking-widest">Cancel Distribution</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-[1000] bg-slate-900/95 backdrop-blur-xl flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-xl rounded-[3.5rem] overflow-hidden shadow-2xl flex flex-col animate-in zoom-in-95 duration-200">
            <div className="p-10 bg-[#0f172a] text-white text-center relative overflow-hidden">
              ...
              <p className="text-[10px] font-black text-indigo-400 uppercase tracking-[0.4em] mb-4">Voice Interaction Session</p>
              ...
              {isCallActive ? (
                <button onClick={endCallSession} className="px-12 py-5 bg-red-600 text-white rounded-2xl font-black text-[11px] uppercase tracking-widest shadow-xl shadow-red-900/40 active:scale-95 transition-all">End Call</button>
              ) : (
                <div className="px-6 py-4 bg-emerald-500/20 border border-emerald-500/30 rounded-2xl">
                  <p className="text-[9px] font-black text-emerald-400 uppercase tracking-widest">{callDuration < MIN_VALID_DURATION ? `Incomplete Session (${callDuration}s)` : 'Interaction Verified'}</p>
                </div>
              )}
            </div>
            ...
            <div className="p-10 space-y-6 bg-white relative">
              {isLocked && (
                <div className="absolute inset-0 bg-white/60 backdrop-blur-[2px] z-10 flex flex-col items-center justify-center text-center p-8">
                  ...
                  <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]">{isCallActive ? 'Finalizing Session...' : `Minimum required: ${MIN_VALID_DURATION}s`}</p>
                </div>
              )}

              {!showDeptPicker ? (
                <div className="space-y-4">
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">Classify Result</p>
                  ...
                  <button onClick={() => setCallingLead(null)} className="w-full py-3 text-slate-300 text-[9px] font-black uppercase tracking-widest mt-2">Dismiss Interface</button>
                </div>
              ) : (
                <div className="space-y-6 animate-in slide-in-from-bottom-4">
                   <div className="text-center">
                     <p className="text-[12px] font-black text-[#0f172a] uppercase tracking-tighter">Academic Interest</p>
                     <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mt-2">Select Preferred Department</p>
                   </div>
